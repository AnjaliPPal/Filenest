        ...
        <div className="max-w-md w-full space-y-8 text-center">
          <div className="animate-pulse">
            ...
            <div className="mt-4 flex justify-center">
              <div className="loader ease-linear rounded-full border-4 border-t-4 border-gray-200 h-10 w-10 border-t-blue-500 animate-spin"></div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="bg-white shadow-sm rounded-lg p-5 border border-gray-100">
          <h3 className="font-medium text-gray-900 mb-2">Why sign in?</h3>
          <ul className="space-y-2 text-sm">
            <li className="flex items-start">
              ...
              <span>Track and manage all your file requests in one place</span>
            </li>
            <li className="flex items-start">
              <svg className="h-5 w-5 text-green-500 mr-2 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
              </svg>
              <span>Automatically receive file notifications via email</span>
            </li>
            <li className="flex items-start">
              <svg className="h-5 w-5 text-green-500 mr-2 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
              </svg>
              <span>Access premium features and increased storage</span>
            </li>
          </ul>
        </div>
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="p-3 bg-gray-50 border border-gray-100 rounded-md text-xs">
            <p>Return URL: {returnUrl || 'None'}</p>
            <p>Auth state: {isAuthenticated ? 'Authenticated' : 'Not authenticated'}</p>
            <p>Context Loading: {contextLoading ? 'Yes' : 'No'}</p>
            <p>Local Loading: {isLoading ? 'Yes' : 'No'}</p>
            <p>Auth Check Done: {authCheckDone ? 'Yes' : 'No'}</p>
          </div>
        )}
        ...
        <div className="mt-6 text-center text-sm text-gray-500">
          ...
          <Link to="/terms" className="font-medium text-gray-600 hover:text-gray-500">
            Terms of Service
          </Link>{' '}
          and{' '}
          <Link to="/privacy" className="font-medium text-gray-600 hover:text-gray-500">
            Privacy Policy
          </Link>
        </div>
      </div>
